require 'prawn' #Ruby library for generating PDF documents
require 'rspec' #For Unit Testing in Ruby
require 'json'  #methods for working with JSON data.
require 'uri'   #methods for working with Uniform Resource Identifiers (URIs).
require 'net/http' #methods for making HTTP requests.
require 'axlsx' #creating Excel files in the XLSX format.

def get_weather(location)
  url = URI("https://weatherapi-com.p.rapidapi.com/current.json?q=#{location}")

  http = Net::HTTP.new(url.host, url.port) #Syntax : http = Net::HTTP.new(host, port) | #object for making HTTP requests.
  http.use_ssl = true   # ssl - to secure server to browser transactions.

  request = Net::HTTP::Get.new(url)  # creates a new GET request object for the specified URL.
  ...
  response = http.request(request) # returns the content of the response sent by the server
  return JSON.parse(response.read_body, symbolize_names: true) #parses json data into ruby Hashes 
end
